...
jQuery(window).scroll(function() {
	...
	let areaTop = jQuery(".l-cta").offset().top - 80; // 対象エリアの上部の位置
	let areaBottom = areaTop + jQuery(".l-cta").innerHeight(); // 対象エリアの下部の位置

	if (scrollTop > areaTop && scrollTop < areaBottom) {
		jQuery("body").addClass("cta_on"); // スクロールが対象エリアに入った場合
	} else {
		jQuery("body").removeClass("cta_on"); // スクロールが対象エリアから出ている場合
	}
});
